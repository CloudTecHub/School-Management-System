...
if (!$student) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($error) && $username !== $current_username) {
            $query = "SELECT staff_id FROM staff WHERE username = ? AND staff_id != ?";
            $stmt = mysqli_prepare($conn, $query);
            mysqli_stmt_bind_param($stmt, "si", $username, $staff['staff_id']);
            mysqli_stmt_execute($stmt);
            $result = mysqli_stmt_get_result($stmt);
        ...
        if (empty($error) && $email !== $staff['email']) {
            $query = "SELECT staff_id FROM staff WHERE email = ? AND staff_id != ?";
            $stmt = mysqli_prepare($conn, $query);
            mysqli_stmt_bind_param($stmt, "si", $email, $staff['staff_id']);
            mysqli_stmt_execute($stmt);
            $result = mysqli_stmt_get_result($stmt);
            if (mysqli_fetch_assoc($result)) {
        ...
        if (empty($error)) {
            ...
            $query = "UPDATE staff SET $fields WHERE staff_id=?";
            $stmt = mysqli_prepare($conn, $query);
